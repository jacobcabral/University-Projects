...
class BST:
    ...
    def __contains__(self, key):
        ...
        b = False
        node = self._root
        while b == False and node is not None:
            if node._value == key:
                b = True
            if node._value < key:
                node = node._right
    ...
    def height(self):
        ...
        if self._count == 0:
            ...
            h = node._right._height + 1
        elif node._left is not None and node._right is None:
            h = node._left._height + 1
        elif node._left._height == node._right._height:
            h = node._left._height + 1
        return h
    ...
    def is_identical(self, other):
        ...
        if self._count == 0 and other._count == 0:
            ...
            node = self._root
            node2 = other._root
            if node._left == node2._left and node._right == other._right:
                identical = True
            else:
                identical = False
        return identical
    ...
    def parent(self, key):
        ...
        if b == False:
            ...
            while len(queue) > 0:
                node = queue.pop(0)
                if key == node._left._value:
                    value = deepcopy(node._value)
                elif key == node._right._value:
                    value = deepcopy(node._value)
    ...
    def is_balanced(self):
        ...
        if self._count == 0:
            ...
            if node._left is None and node._right is not None:
                ...
                val1 = node._left._height
                val2 = node._right._height
                if val1 - val2 <= 1 or val1 - val2 >= -1:
                    balanced = True
                else:
                    balanced = False
        return balanced
    ...
    def is_valid(self):
        ...
        if self.is_balanced():
            ...
            val = node._value
            valid = True
            while node._right is not None and valid == True:
                if val < node._right._value:
                    valid = True
                else:
                    valid = False
                val = node._value
            ...
            node = self._root
            val = node._value
            while node._left is not None and valid == True:
                if val > node._left._value:
                    valid = True
                else:
                    valid = False
                val = node._value
    ...
    def postorder(self):
        ...
        visited = []
        a = []
        while (temp and temp._value not in visited):  
              
            # Visited left subtree  
            if (temp._left and temp._left._value not in visited): 
                temp = temp._left  
            # Visited right subtree  
            elif (temp._right and temp._right._value not in visited): 
                temp = temp._right  
              
            # Print node  
            else: 
                a.append(temp._value)
                visited.append(temp._value)  
                temp = self._root
        return a
