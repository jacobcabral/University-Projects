...
def get_food():
    ...
    # Your code here
    isVeg = True
    foodName = str(input("Name: "))
    foodOrigin = int(input("Origin: "))
    foodVegetarian = str(input("Vegetarian (Y/N): "))
    foodCalories = int(input("Calories: "))
    if foodVegetarian == 'Y':
        isVeg = True
    else:
        isVeg = False
    food = Food(foodName,foodOrigin,isVeg,foodCalories)
    return food
...
def read_food(line):
    ...
    if line[2] == 'True':
        ...
        isVeg = False
    calories = int(line[3])
    food = Food(line[0],origin,isVeg,calories)
    return food
...
def read_foods(file_variable):
    ...
    # Your code here
    foods = []
    read_input = file_variable.readlines()
...
def write_foods(file_variable, foods):
    ...
    for food in foods:
        ...
        file_variable.write(foodString)
...
def food_search(foods, origin, max_cals, is_veg):
    ...
    if origin != -1:
        for j in range(len(foods)):
            if foods[j].origin == origin:
                first.append(foods[j])
    else:
        first = foods
    if max_cals != 0:
        for i in range(len(first)):
            if first[i].calories >= max_cals:
                second.append(first[i])
    else:
        second = first           
    if is_veg != False:
        for g in range(len(second)):
            if second[g].is_vegetarian == is_veg:
                result.append(second[g])
    else:
        result = second
    return result
