...
class Sorted_List:
    ...
    def insert(self, value):
        ...
        """
        if(self._count == 0):
            self._front = self._rear = _SL_Node(value, None)
    ...
    def _linear_search(self, key):
        ...
        if(self._count == 0 ):
            ...
            i = 0
            found = False
            while(i < self._count and found == False):
                if(curr._value == key):
                    found = True
    ...
    def find(self, key):
        ...
        return None if result is None else deepcopy(result._value)
    ...
    def clean(self):
        ...
        node = self._front
        while(node is not None):
            while(node._next is not None and node._next._value == node._value):
    ...
    def intersection(self, source1, source2):
        ...
        node = source1._front
        while(node is not None):
            thisSearch = self._linear_search(node._value)[2]
            if(thisSearch == -1):
                otherSearch = source2._linear_search(node._value)[2]
                if(otherSearch != -1):
                    self._insert(node._value)
    ...
    def union(self, source1, source2):
        ...
        node = source1._front
        while(node is not None):
            search = self._linear_search(node._value)[2]
            if(search == -1):
                self._insert(node._value)
    ...
    def copy(self):
        ...
        if(self._count > 0):
            a = self._front
            b = new_list._front = _SL_Node(deepcopy(a._value), None)
            while(a._next is not None):
                a = a._next
                b._next = _SL_Node(deepcopy(a._value), None)
                b = b._next
